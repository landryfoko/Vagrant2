...
public class DropboxScheduledPollGetConsumer extends DropboxScheduledPollConsumer {
    ...
    protected int poll() throws Exception {
        ...
        if (configuration.getInclude()==null){
        	result = DropboxAPIFacade.getInstance(configuration.getClient()).get(configuration.getRemotePath());
        }else{
        	result = DropboxAPIFacade.getInstance(configuration.getClient()).get(configuration.getRemotePath(), configuration.getInclude());
        }
        result.populateExchange(exchange);
        log.debug("consumer --> downloaded: " + result.toString());
