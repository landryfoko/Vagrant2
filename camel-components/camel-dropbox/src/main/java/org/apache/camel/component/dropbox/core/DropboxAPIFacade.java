...
public class DropboxAPIFacade {
    ...
    public DropboxResult get(String remotePath, String include) throws DropboxException {
        DropboxResult result = new DropboxFileDownloadResult();
        //a map representing for each path the result of the baos
        Map<String, ByteArrayOutputStream> resultEntries = new HashMap<String, ByteArrayOutputStream>();
        //iterate from the remotePath
        downloadFilesInFolder(remotePath, resultEntries, include);
        //put the map of baos as result
        result.setResultEntries(resultEntries);
        return result;
    }
    ...
    private void downloadFilesInFolder(String path, Map<String, ByteArrayOutputStream> resultEntries, String include) throws DropboxException {
        DbxEntry.WithChildren listing = null;
        try {
            listing = instance.client.getMetadataWithChildren(path);
        }
        ...
        if(listing.children == null) {
            log.info("downloading a single file...");
            downloadSingleFile(path,resultEntries,include);
            return;
        }
        for (DbxEntry entry : listing.children) {
            if (entry.isFile()) {
                try {
                    //get the baos of the file
                    downloadSingleFile(entry.path, resultEntries,include);
                }
                catch(DropboxException e) {
                    log.warn("can't download from " + entry.path);
                }
                ...
                //iterate on folder
            	downloadFilesInFolder(entry.path, resultEntries, include);
            }
        }
    }
    ...
    private void downloadSingleFile(String path, Map<String, ByteArrayOutputStream> resultEntries) throws DropboxException {
        ...
        if (downloadedFile != null) {
            ...
            log.debug("downloaded path:"+path+" - baos size:" + baos.size());
        }

    }

    private void downloadSingleFile(String path, Map<String, ByteArrayOutputStream> resultEntries,  String include) throws DropboxException {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        DbxEntry.File downloadedFile = null;
        try {
            downloadedFile = instance.client.getFile(path, null, baos);
        ...
        if (downloadedFile != null) {
        	 ...
        	 if (validate(downloadedFileName,  include)){
        		resultEntries.put(path, baos);
            	log.debug("downloaded path:"+path+" - baos size:" + baos.size());
        	}
        }
//        downloadedFile.asFile().;

    }

	public boolean validate(String fileName, String include) {
		Pattern pattern;
		Matcher matcher;
