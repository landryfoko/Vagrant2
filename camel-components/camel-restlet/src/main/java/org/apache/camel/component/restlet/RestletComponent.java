...
public class RestletComponent extends HeaderFilterStrategyComponent {
    ...
    public RestletComponent(Component component) {
        ...
        //this.component.getDefaultHost().setRequiredScore(1.0f);
        this.component.getDefaultHost().setDefaultMatchingQuery(true);
    }
    ...
    private MethodBasedRouter getMethodRouter(String uriPattern, boolean addIfEmpty) {
        synchronized (routers) {
        	for(String pattern:routers.keySet()){
        		if(pattern.replaceAll("(\\{[^/]*\\})", "{0}").equals(uriPattern.replaceAll("(\\{[^/]*\\})", "{0}"))){
        			routers.put(uriPattern, routers.get(pattern));
        			return routers.get(pattern);
        		}
        	}
