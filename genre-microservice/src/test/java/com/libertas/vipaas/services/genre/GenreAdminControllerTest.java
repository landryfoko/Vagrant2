...
public class GenreAdminControllerTest{
	...
	private static final String ROOT_END_POINT = "/v1/admin/genre";
	...
	@Test
	public void createGenreShouldCreateGenreSuccessfully() throws Exception {		
		...
		doNothing().when(genreService)
			.createGenre(
				(String) genre.get("name"), genre);
		
		mockMvc.perform(post(ROOT_END_POINT +"/{name}", (String) genre.get("name"))
	...
	@Test
	public void createGenreShouldThrowDuplicateGenreExceptionWhenGenreIsAlreadyExist() throws Exception {		
		JSONObject genre = new JSONObject();
		genre.put("id", "sampleid");
		genre.put("deviceId", "sampledeviceId");
		genre.put("name", "samplename");
		...
		doThrow(new DuplicateGenreException("Genre already exists"))
		.when(genreService)
			.createGenre(
				(String) genre.get("name"), genre);
	...
	@Test
	public void deleteGenreByIdShouldDeleteGenreSuccessfully() throws Exception {		
		JSONObject genre = new JSONObject();
		genre.put("id", "sampleid");
		genre.put("deviceId", "sampledeviceId");
		genre.put("name", "samplename");
		...
		doNothing()
		.when(genreService)
		.deleteGenreById((String)genre.get("id"));
		
		mockMvc.perform(delete(ROOT_END_POINT + "/{genreId}", (String) genre.get("id"))
                .contentType(TestUtil.APPLICATION_JSON_UTF8)
                .content(TestUtil.convertObjectToJsonBytes(genre))
        ).andExpect(status().isOk());
	}
	...
	@Test
	public void updateGenreShouldThrowNoSuchGenreExceptionWhenGenreNotFound() throws Exception {		
		JSONObject genre = new JSONObject();
		genre.put("id", "sampleid");
		genre.put("deviceId", "sampledeviceId");
		genre.put("name", "samplename");
		...
		doThrow(new NoSuchGenreException("No such genre"))
		.when(genreService)
		.updateGenre((String)genre.get("id"), genre);
		
		mockMvc.perform(put(ROOT_END_POINT + "/{tenantId}", (String) genre.get("id"))
                .contentType(TestUtil.APPLICATION_JSON_UTF8)
                .content(TestUtil.convertObjectToJsonBytes(genre))
        ).andExpect(status().isNotFound());
	}
